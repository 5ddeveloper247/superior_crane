...
function loadPayDutyPageDataResponse(response) {
    ...
    var forms_list = data.forms_list;

    var total_forms = data.total_forms;
    var total_draft = data.total_draft;
    var total_completed = data.total_completed;
    
    $("#total_forms").text(total_forms);
    $("#total_draft").text(total_draft);
    $("#total_completed").text(total_completed);

    makePayDutyFormsListing(forms_list);
}

function makePayDutyFormsListing(forms_list){

    if ($.fn.DataTable.isDataTable('#payDutyForm_table')) {
        $('#payDutyForm_table').DataTable().destroy();
    }
		...
		$.each(forms_list, function (index, value) {
            ...
            html += `<tr>
                        ...
                        <td class="d-flex gap-2 ">
                            ...
                            ${user_role == '0' ? 
                            `<div class="del deletePayDuty_btn" data-id="${value.id}">
...
$(document).on('click', '.searchPayDuty_btn', function (e) {
   
    let form = document.getElementById('filterPayDuty_form');
    let data = new FormData(form);
    let type = 'POST';
    let url = '/admin/searchPayDutyListing';
    SendAjaxRequestToServer(type, url, data, '', searchTicketListingResponse, '', '.searchPayDuty_btn');
...
function searchTicketListingResponse(response) {
    ...
    if (response.status == 200) {
        var forms_list = response.data.forms_list;
        makePayDutyFormsListing(forms_list);
    }else{
...
$(document).on('click', '.viewPayDuty_btn', function (e) {
    ...
    let type = 'POST';
    let url = '/admin/viewPayDutyFormDetails';
    SendAjaxRequestToServer(type, url, data, '', getSpecificTicketResponse, '', '.viewPayDuty_btn');
...
function getSpecificTicketResponse(response) {
    ...
    if (response.status == 200) {
        ...
        var payduty_detail = response.data.payduty_detail;
        var duty_images = payduty_detail.duty_images;
        
        if(payduty_detail != null){
            $("#pay_date").val(payduty_detail.date);
            $("#pay_location").val(payduty_detail.location);
            $("#pay_start_time").val(payduty_detail.start_time != null ? formatTime(payduty_detail.start_time) : '');
            $("#pay_finish_time").val(payduty_detail.finish_time != null ? formatTime(payduty_detail.finish_time) : '');
            $("#pay_total_hours").val(payduty_detail.total_hours);
            $("#pay_officer_number").val(payduty_detail.officer);
